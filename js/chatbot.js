...
// 입력받은 날짜를 기준으로 일주일간 가장 적게한 카테고리 리턴
...
function getDateFromUser() {
    ...
    $date.textContent = "원하는 날짜를 입력하세요 (YYYY-MM-DD 형식으로 입력해주세요)";
    $response.appendChild($date);
    $date.classList.add('callyResponse');

    const $userDate = $response.lastElementChild.value;
    ...
    if (userDate.length !== 10) {
        ...
        $inputError.textContent = "원하는 날짜를 입력하세요 (YYYY-MM-DD 형식으로 입력해주세요)";
...
// 최근동안 어떠한 일을 많이 했는지 알아봄
function checkRecentWork() {}
// 챗봇에게 할일 추천 받기
function recommendTodo() {
    // 어떤 날짜에 할 일을 추가하고 싶은지 물어봄
    getDateFromUser();

    //입력 받은 날짜를 기반으로 최근동안 어떠한 일을 가장 많이 했는지 알려줌
    checkRecentWork();
